...
const API_URL='https://siva-crm-node.herokuapp.com/requests';
...
export default function ListUsers(){
    ...
    const [getStatus]= useState(['Created','Open','In process','Released','Canceled','Completed']);
    ...
    return(
        ...
        <Modal show={newRequest} onHide={handleClose}>
            ...
            <Modal.Body>
                ...
                <Form.Control value={requirement} name="requirement" onChange={handleInput}></Form.Control>
                <Form.Label>Description</Form.Label>
                <Form.Control value={description} name="description" onChange={handleInput}></Form.Control>
                </Form.Group>
                <Form.Group>
                <Form.Label>Contact</Form.Label>
                <Form.Select value={contact} type="dropdown" name="contact" onChange={handleInput}>
                       {getContacts.map((contact,index)=>{
                           return(
                            <option key={index} value={contact.email}>{contact.email}</option>
                           )
                       })} 
                </Form.Select>
                <Form.Group>
                <Form.Label>Status</Form.Label>
                    <Form.Select value={status} type="dropdown" name="status" onChange={handleInput}>
                        {getStatus.map((eachStatus,index)=>{
                            return( 
                            <option key={index} value={eachStatus}>{eachStatus}</option>
                            );
                        })}
                    </Form.Select>
                </Form.Group>
                <Button type="submit" onClick={addRequest}>{addUpdate}</Button>
            </Form.Group>
                </Form>
            </Modal.Body>
        </Modal>
                ...
                {leadsList.map((lead,index)=>{
                    return(
                        <tr key={index}>
                            ...
                            <th>{lead.contact}</th>
                            <th>{lead.status}</th>
                            <th><Button variant="warning"  onClick={()=>editRequest(lead._id)}>Edit</Button>
                            &nbsp;<Button variant="danger" onClick={()=>deleteRequest(lead._id)}>Delete</Button></th>
                        </tr>
                    )
                })}
            </tbody>
